...
class OverHead(object):
	...
	def crossover(self):
		...
		for i in range(len(self.offspring)):
			self.salesman[i].route = np.copy(self.salesman[self.best_salesmen[i]].route)

		sequence_length = 2

		for i in range(len(self.offspring)):

			# Inheret route, copy from parent, first parent is more likely to have a better route
			first_parent = np.random.randint(len(self.offspring))
			second_parent = np.random.randint(len(self.salesman)-len(self.offspring))
			...
			self.salesman[len(self.salesman) - i - 1].route = np.copy(self.offspring[i].route)
